 ...
 *
 * `share.ts` is an overwrite-synced shared-sync module file, so this suite is
 * the packing-list port of grocery-list's `share.test.ts` (the fleet exemplar).
 * The only app-specific input is `SHARE_SCHEME`, which is read from the
 * app-owned `shareConfig.ts` rather than hardcoded — a scheme change must not
 * need a test edit, and hardcoding it here would pin the wrong app's value.
 */
...
describe('shareConfig', () => {
  it('declares this app\'s own scheme (it must match expo.scheme in app.json)', () => {
    expect(SHARE_SCHEME).toBe('packinglist');
  });
});
...
describe('parseShareLink', () => {
  ...
  it('stops at the next query param — a link with trailing params still pairs', () => {
    expect(parseShareLink(`${SHARE_SCHEME}://join?s=${SECRET}&utm=share`)).toBe(SECRET);
  });
});
...
//
// • parseShareLink `if (!m) return null` → false: with no `s=` match, the
...
//   passes either way.) Carried over from grocery-list, which runs the
//   byte-identical `share.ts`.
// ---------------------------------------------------------------------------
