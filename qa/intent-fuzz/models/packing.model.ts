/**
 * Intent-fuzz MODEL — packing-list trust core (Uplevel 3 / T1). NOT a test file.
 *
 * Drives the REAL `useTripsStore` (a fresh module instance per run via
 * jest.isolateModules; persistence + settings mocked by the CALLING test file so
 * it stays hermetic — Google's "small" test: no disk, no network, no real
 * timers) through random user stories, and after every command asserts INTENT —
 * what a person expects, not "the state happened to converge".
 *
 ...
 *
 * WHY THIS LIVES HERE AND NOT IN THE TEST FILE. The replay kit enumerates EVERY
 * checked-in fixture from every call site, so a `packing` fixture has to be
 * replayable from the trip-sync file too — and a builder that lives inside a
 * `*.test.ts` can't be imported by another test without re-registering that
 * file's whole suite. Keeping the model here (the shape `replay.ts` documents)
 * is what lets one call site register every model. Requiring this module does
 * NOT install the store mocks — the test file that imports it must (see
 * `src/store/__tests__/intentFuzz.test.ts`).
 */

import fc from 'fast-check';
import { intent } from '../harness';
...
import { serializeTrips, parseTransfer } from '../../../src/lib/transfer';

/** The model key crystallized fixtures carry. */
export const PACKING_MODEL = 'packing';
...
/** I-DELETE-STICKS: a row the user deleted stays gone through a recompose that
 *  did not switch a new kit on (defect packing-list-20260908-1). Switching a kit
 *  ON is the one edit that may bring a row back, and only rows no already-chosen
 *  kit produced — so the caller passes `mayRevive: false` for a duration change
 *  and for switching a kit OFF. */
function assertDeletesStuck(before: Trip, after: Trip, ctx: string): void {
  const afterVisible = new Set(visible(after).map((it) => it.id));
  for (const it of before.items) {
    if (it.deletedAt == null) continue;
    intent(
      `${ctx}: deleted item ${it.id} ("${it.name}") came back — a trip edit must never undo a delete`,
      !afterVisible.has(it.id)
    );
  }
}
...
class ToggleType implements fc.Command<Model, Real> {
  ...
  run(m: Model, r: Real): void {
    ...
    if (!before) return;
    const id = before.id;
    const switchingOff = before.typeIds.includes(this.typeId);
    ...
    // Switching a kit ON legitimately restores rows only that kit brings.
    if (switchingOff) assertDeletesStuck(before, after, 'toggle-type-off');
    resync(m, r);
  }
...
class ChangeDuration implements fc.Command<Model, Real> {
  ...
  run(m: Model, r: Real): void {
    ...
    assertDeletesStuck(before, after, 'change-duration');
    resync(m, r);
  }
...
export const packingCommands: fc.Arbitrary<fc.Command<Model, Real>>[] = [
...
export const PACKING_MAX_COMMANDS = 50;

/** A fresh REAL store per run (isolateModules re-instantiates the singleton). */
export function packingSetup(): { model: Model; real: Real } {
  let store!: TripsStore;
  jest.isolateModules(() => {
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    store = require('../../../src/store/trips').useTripsStore;
  });
  return { model: { tripIds: [] }, real: { store } };
}

/** Shared by the live fuzzer and the regression replayer (same property).
 *  `replayPath` is fast-check's commands-level replay token; a crystallized
 *  fixture cannot be walked without it (see `qa/intent-fuzz/replayCommands.ts`),
 *  and the live fuzzer simply passes nothing. */
export function buildPackingProperty(replayPath?: string): fc.IPropertyWithHooks<unknown> {
  return fc.property(
    fc.commands(packingCommands, { maxCommands: PACKING_MAX_COMMANDS, replayPath }),
    (cmds) => {
      const s = packingSetup();
